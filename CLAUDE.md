...
- **`findLocaleClient(translations, intlConfig?)` — synchronous.** Reads stored preference (`document.cookie` or `localStorage` per `intlConfig.storage`) + `navigator.languages`. Sync so it can back `export const t = findLocaleClient(...)` in a module imported by client components without making it async.
- **`findLocaleServer(translations, intlConfig?)` — async.** Dynamically `import("next/headers")` (guarded by try/catch so non-Next servers degrade to `defaultLocale`) and reads the configured cookie + `Accept-Language`. Async because Next 15's `cookies()`/`headers()` are async.
